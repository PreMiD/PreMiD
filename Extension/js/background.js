...
chrome.runtime.onInstalled.addListener(function(details) {
  switch (details.reason) {
    ...
    case "update": {
      ...
      PMD_debug("info", "Extension reloaded");
      updateOptions();
      break;
    }
  }
...
//* Grab Discord Authorization Token
chrome.webRequest.onSendHeaders.addListener(
  function(details) {
    if (details.method == "POST") {
      var authorization = details.requestHeaders.find(
          ...
          if (result.options != undefined) {
            chrome.storage.local.set({
              options: Object.assign(result.options, {
                discordAuthToken: authorization.value
              })
            });
          } else {
            chrome.storage.local.set({
...
PMD_debug("info", "Connecting to Application");
var socket = io.connect("http://localhost:3020/");
socket.on("connect", function() {
  PMD_debug("success", "Connected to Application");
  tabPriorityInterval = setInterval(tabPriority, 1000);
});

socket.on("disconnect", function() {
  PMD_debug("error", "Disconnected from Application");
  clearInterval(tabPriorityInterval);
});

socket.on("mediaKeyHandler", function(data) {
  //* Media control buttons
  PMD_debug("info", `Media Control Keys: ${data.playback}`);
  if (priorityTabId != null)
    chrome.tabs.sendMessage(priorityTabId, { mediaKeys: data.playback });
});

//* Forward the presence data received from Presence script to application
chrome.runtime.onMessage.addListener(function(data, sender) {
  if (data.presence != undefined) {
    PMD_debug("info", `Sending Presence Data to Application`);
    socket.emit("updateData", data.presence);
  }
  if (data.iframe_video != undefined && priorityTabId != null) {
    PMD_debug("info", `Sending iFrame video data to presence`);
...
async function tabPriority() {
  chrome.storage.local.get(["presences"], function(result) {
    if (result.presences != undefined) {
      allowedTabsStatic = [];
      result.presences.forEach(presence => {
        if (presence.enabled) allowedTabsStatic.push(presence.url);
      });
      allowedTabs = allowedTabsStatic.slice();
              ...
              if (tabs[0].url.indexOf(allowedTabs[i]) > -1) {
                if (lastTabPriorityLock == 5) {
                  if (priorityTabId != tabs[0].id) {
                    if (priorityTabId != null) {
                      chrome.tabs.sendMessage(priorityTabId, {
                        tabPriority: false
                      });
                    }
...
async function updateOptions() {
  chrome.storage.sync.get(["options"], async function(result) {
    ...
    options[checkStorage("debugging", options)];
...
chrome.tabs.onUpdated.addListener(function(tabs) {
  chrome.storage.local.get(["presences"], function(data) {
    ...
    presences.forEach(presence => {
      chrome.tabs.query({ active: true }, function(tabs) {
        if (
          tabs[0].url.includes(presence.url) &&
          presence.enabled &&
          ...
          chrome.tabs.executeScript(
            ...
            function(result) {
              ...
              if (result[0] == null) {
                PMD_debug("info", `Injecting ${presence.name}...`);
                currentTabURL = tabs[0].url;
                chrome.tabs.executeScript(
                  tabs[0].id,
                  {
...
function getAuthors() {
  ...
  return songAuthorsString
                      ...
                      function(res) {
                        PMD_debug("success", "Inject successful!");
                      }
                    );
                  }
                );
