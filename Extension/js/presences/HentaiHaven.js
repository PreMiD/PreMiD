...
function updateData(playbackChange = false) {
  var videoTitle = $('.entry-title').text().split(" – ")[0]
  var videoAuthor = $('.entry-title').text().split(" – ")[1]
  var eventType,
  videoRunning = $('.entry-title').text() != "" && $('.vjs-tech').get(0) != undefined && !isNaN($('.vjs-tech').get(0).duration) ? true : false
  if(videoRunning) {
