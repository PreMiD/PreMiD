...
function updateData(playbackChange = false) {
  ...
  var videoTitle = $('.boxBarraInfo.playBoxAncora h1').text().split(" Episódio ")[0]
  var videoAuthor = getString("presence.episode") + " " + $('.boxBarraInfo.playBoxAncora h1').text().split(" Episódio ")[1]
  var eventType,
  videoRunning = $('.boxBarraInfo.playBoxAncora h1').text() != "" && $('.vjs-tech').get(0) != undefined && !isNaN($('.vjs-tech').get(0).duration) ? true : false
  if(videoRunning) {
