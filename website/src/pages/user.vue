...
export default {
  ...
  mounted(){
    ...
    scr.innerHTML = "$.getJSON('http://paz.yt:6942/avatar/'+location.search.replace('?', ''), function(d){document.querySelector('#avatar').src=d.url;});$('#avatar').on('load', function(){document.querySelector('#avatar').style.transform='scale(1)';});$.getJSON('https://api.premid.app/getServices', function(d){for(i = 0; i < d.length; i++){if(location.search.replace('?', '') == d[i].userID){var div = document.createElement('div');div.innerHTML = '<div class=\"col s6 m4\"> <div class=\"card\"> <a data-position=\"top\" class=\"tooltipped\" onclick=\"sendAddon(\\''+d[i].id+'\\',\\''+d[i].name+'\\');\" data-tooltip=\"Add to PreMiD\" href=\"#\"><div class=\"card-image\"> <img src=\"https://raw.githubusercontent.com/Timeraa/PreMiD/V1.4/presences/'+d[i].name+'/presence.png\"> <span class=\"card-title\">'+d[i].name+'</span> </div></a><div class=\"card-content\"> <p>'+d[i].description+'</p></div><div class=\"card-action\"> <a href=\"http://'+d[i].url+'\">'+new XMLSerializer().serializeToString(document.querySelector('#external-link-alt').firstChild)+' VIEW SITE</a> </div></div></div>';document.querySelector('#apps').appendChild(div);}$('#loader').remove();var datakey=[];for(data in document.querySelector(\"#view\").dataset){datakey.push(data)};$(\"*\").attr(\"data-\"+datakey[0].replaceAll(\"v\", \"v-\").replaceAll(\"v--\", \"v-\"), \"\");var instances = M.Tooltip.init(document.querySelectorAll('.tooltipped'));}});";
