...
const resolver: QueryResolvers["presences"] = async (_parent, { author, contributor, limit, query, service, start, tag }, _context, info) => {
	...
	const contributorFilter: FilterQuery<PresenceSchema> = contributor ? { "metadata.contributors.name": contributor } : {};
	const serviceFilter: FilterQuery<PresenceSchema> = service ? (Array.isArray(service) ? { "metadata.service": { $in: service } } : { "metadata.service": service }) : {};
	const queryFilter: FilterQuery<PresenceSchema> = query ? { "metadata.service": { $options: "i", $regex: query } } : {};
	const tagFilter: FilterQuery<PresenceSchema> = tag ? { "metadata.tags": tag } : {};
