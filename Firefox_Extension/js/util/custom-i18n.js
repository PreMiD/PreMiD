...
detectLanguage = browser.i18n.getUILanguage()
...
async function loadLanguageFile(languageCode) {
  ...
  defaultLanguageFile = await fetch(browser.runtime.getURL('languages/en.json'))
  .then(response => response.json())
  .then(responseData => {
    return responseData
  })
  //* Load actual language file to try to get String from
  return await fetch(browser.runtime.getURL('languages/' + languageCode + '.json'))
  .then(response => response.json())
  .then(responseData => {
    return responseData
  })
